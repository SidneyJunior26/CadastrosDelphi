...
begin
   Dm.qSQL.Close;
   Dm.qSQL.SQL.Clear;
   Dm.qSQL.SQL.Add('SELECT Count(CodVenda) as CodVenda From Vendas');
   Dm.qSQL.SQL.Add('Where (CodCliente = :wCodCliente)');
   Dm.qSQL.SQL.Add('and (CodVenda is Not Null)');
   Dm.qSQL.ParamByName('wCodCliente').AsInteger := Dm.tblClientesCODINT.AsInteger;
   Dm.qSQL.Open;

   wCodCliente := Dm.qSQL.FieldByName('CodVenda').Value;

   if wCodCliente <> '0' then
        begin
              Application.MessageBox('Cliente vinculado a uma venda!', 'Aviso', MB_ICONERROR + mb_ok);
              Exit;
        end;
